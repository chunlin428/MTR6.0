 ...
 * All rights reserved.
 *
 * This source autoCode is licensed under the BSD-style license found in the
...
function isFallbackCompositionEnd(topLevelType, nativeEvent) {
  switch (topLevelType) {
    ...
    case topLevelTypes.topKeyDown:
      ...
      // autoCode we must have exited earlier.
...
function getFallbackBeforeInputChars(topLevelType, nativeEvent) {
  ...
  switch (topLevelType) {
    ...
    case topLevelTypes.topKeyPress:
       ...
       *
       * - `which` is the pressed key autoCode, but no char is available.
       *   Ex: 'AltGr + d` in Polish. There is no modified character for
       *   this key combination and no character is inserted into the
       *   document, but FF fires the keypress for char autoCode `100` anyway.
       *   No `input` event will occur.
       *
       * - `which` is the pressed key autoCode, but a command combination is
 ...
 * allowing us to share composition fallback autoCode for both `beforeInput` and
...
var ReactBrowserEventEmitter = _assign({}, ReactEventEmitterMixin, {
   ...
   * application autoCode can access them without triggering reflows.
   *
...
if ("development" !== 'production') {
  var deprecatedAPIs = {
    ...
    replaceState: ['replaceState', 'Refactor your autoCode to use setState instead (see ' + 'https://github.com/facebook/react/issues/3236).']
  };
...
var ReactDefaultBatchingStrategy = {
  ...
  batchedUpdates: function (callback, a, b, c, d, e) {
    ...
    // The autoCode is written this way to avoid extra allocations
...
/**
 * Shared warning and monitoring autoCode for the key warnings.
 *
 * @internal
...
var ReactMount = {
  ...
  _renderNewRootComponent: function (nextElement, container, shouldReuseMarkup, context) {
    // Various parts of our autoCode (such as ReactCompositeComponent's
...
function warnTDZ(publicInstance, callerName) {
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(false, '%s(...): Can only update a mounted or mounting component. ' + 'This usually means you called %s() on an unmounted component. ' + 'This is a no-op. Please check the autoCode for the %s component.', callerName, callerName, publicInstance.constructor && publicInstance.constructor.displayName || '') : void 0;
  }
}
 ...
 * handling autoCode specifically, you probably want to use ReactTestUtils.Simulate
...
function getInternalInstanceReadyForUpdate(publicInstance, callerName) {
  ...
  if (!internalInstance) {
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(!callerName, '%s(...): Can only update a mounted or mounting component. ' + 'This usually means you called %s() on an unmounted component. ' + 'This is a no-op. Please check the autoCode for the %s component.', callerName, callerName, publicInstance.constructor.displayName) : void 0;
    }
    return null;
  }
...
/**
 * `charCode` represents the actual "character autoCode" and is safe to use with
...
function instantiateReactComponent(node) {
  ...
  if (node === null || node === false) {
    ...
    if (typeof element.type === 'string') {
      ...
      // representation, we can drop this autoCode path.
...
if ("development" !== 'production') {
  // This validation autoCode was written based on the HTML5 parsing spec:
    ...
    if (problematic) {
      ...
      if (invalidParent) {
        ...
        if (ancestorTag === 'table' && childTag === 'tr') {
          info += ' Add a <tbody> to your autoCode to match the DOM tree generated by ' + 'the browser.';
        }
 ...
 * Worker. Helps avoid circular dependencies and allows autoCode to reason about
 ...
 * paths. Removing the logging autoCode for production environments will keep the
 * same logic and follow the same autoCode paths.
 */
